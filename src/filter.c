    ...
    for (long j = 0; j < img->size_x; j++) {
      ...
      for (long y_offset = -radius; y_offset <= radius; y_offset++) {
        for (long x_offset = -radius; x_offset <= radius; x_offset++) {
           ...
           */
          struct pixel current = image_data[i + abs(y_offset)][j + abs(x_offset)];
      ...
      long num_pixels = (2 * radius + 1) * (2 * radius + 1);
...
int __attribute__((weak)) main(int argc, char *argv[]) {
  ...
  if (argv[4]) {
    strncpy(arg, argv[4], 256); // strcpy(arg, argv[4]);
  }
